...
if(!function_exists('businessadmin_activityLog')){
	function businessadmin_activityLog($module, $requested_path = NULL,$requested_string = NULL) {
		...
		$businessuser_id = session('user_id');
		$data = array(
...
if(!function_exists('get_product_cat_name')){
	function get_product_cat_name($pk_id="")
	{
		$db    = \Config\Database::connect();
		$query = $db->query("SELECT bc.title FROM business_categories bc
		join product_categories pc on pc.business_category = bc.pk_id
		where pc.pk_id=$pk_id");
		$product_unit= $query->getRowArray();
		return $product_unit['title'];
	}
}
